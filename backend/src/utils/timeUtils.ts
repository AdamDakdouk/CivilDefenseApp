...
export function calculateHours(startTime: string, endTime: string): number {
  ...
  // Time-only calculation (assumes same day or assumes next day if time wraps)
  let diff = endMinutes - startMinutes;
  
  // If times are exactly the same, treat as 24-hour shift
  if (diff === 0 && startTimeOnly === endTimeOnly) {
    return 24;
  }
